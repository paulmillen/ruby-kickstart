...
def shared(a, b)
  ...
  a.each {|a| return_hash[a] = nil }
  b.each {|b| return_hash[b] = nil }

  return_hash.each do |key,value|
    a.include?(key) ? return_hash[key] = [true] : return_hash[key] = [nil]
    b.include?(key) ? return_hash[key] << true : return_hash[key] << nil
  end
